...
template <typename T>
double epot(T* tin, int nmax, double p)
{
 double epot=0;
for(int i = -nmax ; i <= nmax ; ++i)
{ 
...
template <typename T>
void F(T* t1,T* t2, int nmax, double p, double h)
{
  ...
  for(int i=-nmax; i<=nmax;++i)
  {
    ...
    sb=0;    
  	...
  	t2[nmax+i]=h*(i*i*t1[3*nmax+1+i]+p*sa);
  	t2[3*nmax+1+i]=-h*(i*i*t1[nmax+i]+p*sb);
...
cout<<setprecision(9)<<ekin(sol[0],nmax)+epot(sol[0],nmax,p)<<endl;
cout<<setprecision(9)<<ekin(sol[steps-3],nmax)+epot(sol[steps-3],nmax,p)<<endl;
